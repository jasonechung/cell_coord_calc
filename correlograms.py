...
def compute_correlograms(sorting,
                         ...
                         symmetrize=False, shuffle=False): 
    """
    ...
    spikes = [(sorting.get_all_spike_trains(outputs='unit_index')[0][0].flatten(), sorting.get_all_spike_trains(outputs='unit_index')[0][1].flatten())] 
    ...
    if shuffle:
        ...
        for label in np.unique(labels):
            unit_st=times[np.where(labels==label)]
            offset = np.random.randint(ts,te)
            unit_st = unit_st + offset
            unit_st[unit_st>te] = unit_st[unit_st>te] - te + ts #circular shuffle
            times[np.where(labels==label)] = unit_st
        labels = labels[np.argsort(times)]
        times = times[np.argsort(times)]
        spikes[0] = (times, labels)
